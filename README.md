...
* -n --dist_tipo permette di selezionare la distribuzione di probabilità dalla quale vengono generate frequenze, è possibile scegliere tra "1: 1/nu, 2: 1/(nu^2), 3: Gaussiana" (default=1)
* -d --rel_disp permette di selezionare la relazione di dispersione del pacchetto. "1: w = sqrt(bk), 2: w = sqrt(ck), 3: w = sqrt(c(k^3)), 4 per w = sqrt(b + c(k^2))" (default=1)
...
```
## Requisiti
Per la corretta esecuzione del programma è necessario installare i moduli presenti nel file: "requirements.txt". Per farlo è sufficiente scaricare il file e utilizzare il comando 
```bash
