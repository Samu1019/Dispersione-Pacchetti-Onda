...
if __name__ == "__main__":
    ...
    if (args.evolve):             #Evoluzione dei vari pacchetti d'onda per 4 istanti di tempo nell'intervallo [0,200]s
        if (not packets):
            ...
            sys.exit(1)
        t_f=200
        t_istanti=np.linspace(0, t_f, 4)
        x_ax = np.linspace(-20, 1000, 20000)
        for p in packets:
            fig, axs = plt.subplots(2, 2, figsize=(10, 8), sharex=True, sharey=True)
            fig.suptitle(fr"Evoluzione Pacchetto: N = {p.N},(Relazione di Dispersione {rel_disp}), Distribuzione di frequenza={labels_dist.get(distr_freq, '')}", fontsize=14)
    ...
    if(args.fourier):             #Analisi dello spettro di fourier per i pacchetti generati, in x=0
        ...
        dt = t_fft[1] - t_fft[0]
        for p in packets:
            psi_start=evolve_wp(p,0,t_fft,rel_disp,b,c)
            c_k = fft.rfft(psi_start) / len(psi_start)
            ...
            f_plot = freq[mask]
            p_plot = pow[mask]
            plt.plot(freq[mask], pow[mask], color='crimson', lw=0.6, alpha=0.8,label='Spettro FFT')
            plt.title(fr"Analisi di Fourier N={p.N}, Distribuzione di frequenza={labels_dist.get(distr_freq, '')}")
